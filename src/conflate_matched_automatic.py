...
def do_opstina(data_path, street_mappings, opstina):
    ...
    for naselje_name, df_naselje in df_opstina.groupby('rgz_naselje'):
        ...
        api = osmapi.OsmApi(passwordfile='osm-password', changesetauto=True, changesetautosize=20, changesetautotags={
        ...
        for _, address in only_matched_addresses.sort_values(['rgz_ulica', 'rgz_kucni_broj']).iterrows():
            ...
            while True:
                response = input(
                    f"({i}/{len(df_naselje)}) Are you sure you want to add ref:RS:kucni_broj={rgz_kucni_broj} to {entity['tag']['addr:street']} {entity['tag']['addr:housenumber']} (Y/n/c)?")
