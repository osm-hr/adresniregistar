...
def main():
    ...
    df_addresses = pd.read_csv(os.path.join(osm_path, 'addresses.csv'), dtype={settings.STREET_REF_TAG: 'string', settings.HOUSE_REF_TAG: 'string', 'osm_postcode': 'string'})
    ...
    addresses_with_opstina[settings.STREET_REF_TAG] = addresses_with_opstina[settings.STREET_REF_TAG].astype('str')
