...
def load_removed_addresses_from_osm(cwd):
    ...
    crwh = CollectRelationWaysHandler('removed:'+settings.HOUSE_REF_TAG)
    crwh.apply_file(pbf_file)
    print(f"Collected all ways ({len(crwh.ways)}) from relations")

    cwnh = CollectWayNodesHandler(crwh.ways, 'removed:'+settings.HOUSE_REF_TAG)
    ...
    ceh = CollectEntitiesHandler(bnch.nodes_cache, cwnh.ways_cache, 'removed:'+settings.HOUSE_REF_TAG, collect_tags=True)
